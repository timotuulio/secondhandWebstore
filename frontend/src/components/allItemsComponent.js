...
*/
    ...
    function deleteItem(e) {
        ...
        }).then(res => res.json()).then(data => loadingAction(), alert3());
    }
    ...
    if (page === 'OWNSELLABLES' || page === 'OFFERS' || page === 'STOCK') {
        fetch('http://localhost:3001/api/' + currentPath, {
            headers: {
                'Content-type': 'application/json',
                'Authorization': 'Bearer ' + token
            }
        }).then(res => res.json()).then(data => setData(data)).then(loadedAction);
    ...
    const itemsToRender = [<div key={33} style={{
        ...
        <Alert isOpen={visible2} color="success">
    ...
    if (loadState === 'LOADED') {
        ...
        item.map(itm => itemsToRender.push(<div key={itm._id} style={{
                display: "flex",
                margin: "15px",
                justifyContent: "center",
                alignItems: "center",
...

                {
                    (() => {
                        if (page === 'OWNSELLABLES' || page === 'MAIN') {
                            ...
                            return <CardHeader>{itm.title}<Button color="primary" style={{
                                    float: 'right'
                                }} onClick={mainAction}>Seller</Button>
                            </CardHeader>
                ...
                <CardFooter>
                    {
                        (() => {
                            if (page === 'OWNSELLABLES') {
                                return <div>
                                    <Button color="primary" value={itm._id} onClick={deleteItem}>Remove</Button>
                                    <Button color="primary" style={{
                                            float: 'right'
                                        }} onClick={() => editItemAction(itm)}>Edit</Button>
                                </div>
                            } else if (page === 'STOCK') {
                                return <div>
                                    <Button color="primary" onClick={() => addForSaleAction(itm)}>Add to sales</Button>
                                </div>
                            } else if (login === 'LOGGEDIN') {
                                return <div>
                                    <Button color="primary" value={itm._id} onClick={buyItem}>Buy</Button>
                                </div>
                            }
                        })()
                    }
        ...
        itemsToRender.push(<div key="none">
            <div>Loading...</div>
        </div>)
    }
...
*/
    ...
    if (page === 'OWNSELLABLES') {
        return (<div>
            <div>
                <h2 className="display-4">Own sellables</h2>
                <hr className="my-2"/></div>
            <div>{itemsToRender}</div>
        </div>)
    } else if (page === 'MAIN') {
        return (<div>
            <h2 className="display-4">Shop goods</h2>
            <hr className="my-2"/>
            <div>{itemsToRender}</div>
        </div>)
    } else if (page === 'STOCK') {
        return (<div>
            <h2 className="display-4">Stock</h2>
            <hr className="my-2"/>
            <div>{itemsToRender}</div>
        </div>)
    } else {
        return (<div>
            <h2 className="display-4">Offers</h2>
            <hr className="my-2"/>
            <div>{itemsToRender}</div>
        </div>)
    }
...
const mapStateToProps = (state) => ({loadState: state.loadReducer.loadState, user: state.loginReducer.user, token: state.loginReducer.token, page: state.pageReducer.page, login: state.loginReducer.login});
