...
const Profile = ({mainAction, token, user, updateProfileAction}) => {
    ...
    let submit = (e) => {
        ...
        fetch(url, {
            ...
            body: body
        }).then(response => response.text()).then(res => updateProfileAction(JSON.parse(res), alert2()))
    ...
    function removeAccount() {
        ...
        }).then(response => response.text()).then(res => mainAction(), loggedOutAction())
    }
    ...
    return (<div style={{
        ...
        <Form style={{
            ...
            <Card className="text-center">
                <CardHeader>
                    <h3 className="display-5">{user.email}
                        / Balance: {user.balance}
                        €</h3>
                    <Button onClick={addBalanceAction} color="primary">Add balance</Button>
                ...
                <div>
                    <div>
                        <Button color="primary" size="lg" onClick={submit}>Save changes</Button>
                    </div><br/><br/>
